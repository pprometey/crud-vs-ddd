...
public class MarkNoShowHandlerTests
{
    ...
    private readonly MarkNoShowHandler _handler;

    public MarkNoShowHandlerTests()
    {
        _appointmentRepo = new FakeAppointmentRepository();
        _uow = new FakeUnitOfWork();
        _publisher = new FakePublisher();
        _handler = new MarkNoShowHandler(_appointmentRepo, _uow, _publisher);
    }
    ...
    [Fact]
    public async Task Handle_ConfirmedAppointment_MarksAsNoShow()
    {
        // Arrange
        var appointment = CreateConfirmedAppointment();
        var command = new MarkNoShowCommand(appointment.Id.Value);
        ...
        Assert.Equal(AppointmentStatus.Cancelled, updated.Status); // no-show → cancelled
    }

    [Fact]
    public async Task Handle_ValidNoShow_CallsSaveChanges()
    {
        // Arrange
        var appointment = CreateConfirmedAppointment();
        var command = new MarkNoShowCommand(appointment.Id.Value);
    ...
    public async Task Handle_AppointmentNotFound_ReturnsFailure()
    {
        ...
        var command = new MarkNoShowCommand(nonExistentId);
    ...
    [Fact]
    public async Task Handle_CompletedAppointment_ThrowsDomainException()
    {
        // Arrange
        var appointment = CreateConfirmedAppointment();
    ...
    [Fact]
    public async Task Handle_NoShow_RegistersAppointmentNoShowEvent()
    {
        // Arrange
        var appointment = CreateConfirmedAppointment();
        _publisher.Clear();
        ...
        // Assert
        var noShowEvent = _publisher.PublishedEvents.OfType<AppointmentNoShow>().FirstOrDefault();

        Assert.NotNull(noShowEvent);
        Assert.Equal(appointment.Id, noShowEvent.AppointmentId);
        Assert.Equal(appointment.PatientId, noShowEvent.PatientId);
    }
}
